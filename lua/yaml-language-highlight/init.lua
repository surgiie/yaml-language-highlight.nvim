...
--- `stdpath("config")/queries/yaml/injections.scm`.
--- ~/.config/nvim is always first on the runtimepath and is never managed
--- by plugin managers, so creating a real directory here is safe.
---
--- @param injections table<string, string[]>  e.g. { php = { "php", "php_script" } }
local function write_injections(injections)
	local dir = vim.fn.stdpath("config") .. "/queries/yaml"
	...
	if not base:match("^; extends") then
		base = "; extends\n" .. base
	end
